...
import { getReports, generateReportTitle } from '../../../../lib/reportApi';
...
const ReportBoard = () => {
  ...
  const loadReports = async () => {
    try {
      ...
      const response = await getReports(currentPage, REPORTS_PER_PAGE, searchTerm);
      ...
      const formattedData = response.data.map((report, index) => ({
        id: report.id,
        number: response.total - ((currentPage - 1) * REPORTS_PER_PAGE) - index,
        title: generateReportTitle(report.apartment_name, report.building_number, report.phone_last),
        date: new Date(report.created_at).toLocaleDateString('ko-KR'),
  ...
  return (
    <Container>
      <Content>
        <Header>
          <Title>점검 보고서 목록</Title>
          <Subtitle>고객님 전화번호 뒤 4자리를 검색창에 입력하여 보고서를 확인해보세요.<br/>
        ...
        {reports.length === 0 ? (
          <EmptyMessage>등록된 보고서가 없습니다.</EmptyMessage>
        ) : (
          <>
            <BoardTable>
              <TableHeader>
              ...
              <TableBody>
                {reports.map((report) => (
                  <TableRow key={report.id} onClick={() => handleReportClick(report.id)}>
                    ...
                    <TableCell className="title">{report.title}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </BoardTable>
